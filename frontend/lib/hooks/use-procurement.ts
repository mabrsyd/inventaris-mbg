...
export function useSubmitPurchaseOrder() {
  ...
  return useMutation({
    mutationKey: ['purchase-orders', 'submit'],
    ...
    onError: (error: any) => {
      toast({
        title: 'Error',
        description: error.response?.data?.message || 'Failed to submit purchase order',
        variant: 'destructive',
      });
    },
  });
...
export function useApprovePurchaseOrder() {
  ...
  return useMutation({
    mutationKey: ['purchase-orders', 'approve'],
...
export function useRejectPurchaseOrder() {
  ...
  return useMutation({
    mutationKey: ['purchase-orders', 'reject'],
...
export function useCreateGoodsReceipt() {
  ...
  return useMutation({
    mutationKey: ['goods-receipts', 'create'],
...
export function useUpdateGoodsReceipt() {
  ...
  return useMutation({
    mutationKey: ['goods-receipts', 'update'],
...
export function useDeleteGoodsReceipt() {
  ...
  return useMutation({
    mutationKey: ['goods-receipts', 'delete'],
...
export function useUpdateQcStatus() {
  ...
  return useMutation({
    mutationKey: ['goods-receipts', 'update-qc'],
    mutationFn: ({
      id,
      itemId,
      qcStatus,
...
/**
 * Update goods receipt QC status
 */
export function useUpdateGoodsReceiptQC() {
  const queryClient = useQueryClient();
  const router = useRouter();
  ...
  return useMutation({
    ...
    mutationFn: ({ 
      ...
      qcNotes?: string;
      qcDate?: string;
    }) => goodsReceiptsApi.updateQC(id, qcStatus, qcNotes, qcDate),
    onSuccess: (response, variables) => {
      // Invalidate both list and specific item queries
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.GOODS_RECEIPTS] });
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.GOODS_RECEIPTS, variables.id] });
      // Force refetch the specific goods receipt
      queryClient.refetchQueries({ queryKey: [QUERY_KEYS.GOODS_RECEIPTS, variables.id] });
      toast({
        title: 'Success',
        description: 'QC status updated successfully',
      });
